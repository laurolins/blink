...
class TetrahedronToEPS {
    float CONVERSION = 72f / 2.54f;

    // all of these are in cm
    private double _width = 18;
    private double _height = 8.5;
    private double _margin = 1;

    private RTetrahedron _tet;

    HashMap<GemVertex, Point2D.Double> _map;

    public TetrahedronToEPS(RTetrahedron tet, int level) throws FileNotFoundException {
        _tet = tet;

        // create first page
        PrintWriter pw = new PrintWriter(String.format("c:/workspace/blink/tex/tet" + tet.getLabel() + "-" + level + ".eps"));
        this.printHeader(pw);
        ...
        pw.println(String.format("(%s) show", "Tetrahedron "+tet.getLabel()+" at level "+level));
        pw.println("grestore");

        GemColor pairs[][] = { {GemColor.yellow, GemColor.blue}, {GemColor.yellow, GemColor.red}, {GemColor.yellow,
                             GemColor.green}, {GemColor.blue, GemColor.red}, {GemColor.blue, GemColor.green},
    ...
    private void draw(
        ...
        RFace f1 = _tet.getFace(c1);
        RFace f2 = _tet.getFace(c2);
        ...
        map.put(u, Vector.valueOf(new double[] {0, sy / 2.0}));
        map.put(v, Vector.valueOf(new double[] {sx, sy / 2.0}));
        map.put(x, Vector.valueOf(new double[] {sx / 2.0, 0}));
        map.put(y, Vector.valueOf(new double[] {sx / 2.0, sy}));
    ...
    private void tutteLayout(HashSet<RPoint> P, HashSet<RSegment> S, HashMap<RPoint,Vector> map) {
        ...
        ArrayList<RPoint> listVertices = new ArrayList<RPoint>(P);
