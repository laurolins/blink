 ...
 * @version 1.0
 */
public class GemExhaustiveSimplifier {
    Gem _gem;
    long _maxTime;
    long _startingTime;
    ...
    public GemExhaustiveSimplifier(Gem g, long maxTime) {
        _gem = g;
        _maxTime = maxTime;
        ...
        // for all gems in the current TS-Class
        // do the all possible U-Moves
        int k=0;
        for (Gem gg : _currentTSClass) {
            ...
            int n = gg.findMonopoles().size();

            System.out.println("TS-Rep "+(++k)+"     u-moves: "+n);

            // for all monopoles in the current
            // tsClass gem do a U-Move
            for (int i = 0; i < n; i++) {
                Gem ggAux = gg.copy();
                System.out.println("U-Move "+i);

                Monopole monopole = ggAux.findMonopoles().get(i);
                ...
                ggAux.uMove(monopole);

                // node already on the tree
                if (_map.get(ggAux) != null)
                    continue;

                // add new vertex on the reduction graph
                Vertex vv = _G.addVertex(new SparseVertex());
                vv.setUserDatum("key", ggAux, UserData.SHARED);
                ...
                // set current gem for processing
                _currentGem = ggAux;
    ...
    public static void main(String[] args) throws ClassNotFoundException, IOException, SQLException {
        ...
        PrintWriter pw = new PrintWriter("c:/equiv107.txt");
        for (int i=0;i<1;i++) {
            Gem g2 = map.get(256L);

            GemExhaustiveSimplifier A = new GemExhaustiveSimplifier(g2,1000000L);
            Gem best = A.getBestAttractorFound();
            System.out.println(""+best.getCurrentLabelling().getLettersString(","));
            for (GemEntry e: list) {
                if ((new Gem(e.getLabelling())).compareTo(best) == 0) {
                    System.out.println("gem = " + e.getId());
                    pw.println("gem = " + e.getId());
                    pw.flush();
                }
            }
